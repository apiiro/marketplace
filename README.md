...
The response body is written to stdout — pretty-printed on a terminal, raw when piped, so it composes with `jq`. Exit code is 0 for 2xx responses and 1 otherwise; on failure the error body still goes to stdout and a summary line goes to stderr. The path must be relative to the API URL, and the `Authorization` header is always managed by the CLI.
